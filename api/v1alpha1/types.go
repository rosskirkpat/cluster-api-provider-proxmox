...
const (
	...
	AnnotationClusterInfrastructureReady = "proxmox.infrastructure.cluster.x-k8s.io/infrastructure-ready"

	// AnnotationControlPlaneReady indicates the cluster's control plane is
	// ready.
	AnnotationControlPlaneReady = "proxmox.infrastructure.cluster.x-k8s.io/control-plane-ready"
...
const (
	...
	// Windows indicates the VM uses Windows Server as the OS.
	Windows OS = "Windows"
)
...
type VirtualMachineCloneSpec struct {
	...
	// without TLS certificate validation of the communication between Cluster API Provider Proxmox
	// and the Proxmox server.
	// +optional
	...
	// Defaults to the eponymous property value in the template from which the
	// virtual machine is cloned.
	// Check the compatibility with the Proxmox version before setting the value.
	// +optional
	HardwareVersion string `json:"hardwareVersion,omitempty"`
}

// ProxmoxMachineTemplateResource describes the data needed to create a ProxmoxMachine from a template
...
const (
	...
	MachineCreated ProxmoxMachineProviderConditionType = "MachineCreated"
)
...
type NetworkDeviceSpec struct {
	// NetworkName is the name of the Proxmox network to which the device
	// will be connected.
	...
	// DHCP6 is a flag that indicates whether to use DHCP for IPv6
	// on this device.
...
type NetworkStatus struct {
	...
	// IPAddrs is one or more IP addresses reported by Proxmox.
	// +optional
	IPAddrs []string `json:"ipAddrs,omitempty"`
