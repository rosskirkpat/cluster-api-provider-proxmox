...
func GetProxmoxMachinesInCluster(
	...
	namespace, clusterName string) ([]*infrav1.ProxmoxMachine, error) {
	...
	machines := make([]*infrav1.ProxmoxMachine, len(machineList.Items))
...
// GetProxmoxMachine gets a proxmox.infrastructure.cluster.x-k8s.io.ProxmoxMachine resource for the given CAPI Machine.
func GetProxmoxMachine(
	ctx context.Context,
	controllerClient client.Client,
	namespace, machineName string) (*infrav1.ProxmoxMachine, error) {
...
// ProxmoxMachine resource.
func GetMachinePreferredIPAddress(machine *infrav1.ProxmoxMachine) (string, error) {
	var cidr *net.IPNet
...
// string for a given ProxmoxMachine.
// IPAM state includes IP and Gateways that should be added to each device.
func GetMachineMetadata(hostname string, proxmoxVM infrav1.ProxmoxVM, ipamState map[string]infrav1.NetworkDeviceSpec, networkStatuses ...infrav1.NetworkStatus) ([]byte, error) {
	// Create a copy of the devices and add their MAC addresses from a network status.
	devices := make([]infrav1.NetworkDeviceSpec, integer.IntMax(len(proxmoxVM.Spec.Network.Devices), len(networkStatuses)))

	var waitForIPv4, waitForIPv6 bool
	for i := range proxmoxVM.Spec.Network.Devices {
		proxmoxVM.Spec.Network.Devices[i].DeepCopyInto(&devices[i])
		...
		// check static IPs
		for _, ipStr := range proxmoxVM.Spec.Network.Devices[i].IPAddrs {
		...
		if proxmoxVM.Spec.Network.Devices[i].DHCP4 {
			waitForIPv4 = true
		}
		if proxmoxVM.Spec.Network.Devices[i].DHCP6 {
			waitForIPv6 = true
		}
	}
	...
	if err := tpl.Execute(buf, struct {
		...
		Devices:     devices,
		Routes:      proxmoxVM.Spec.Network.Routes,
		WaitForIPv4: waitForIPv4,
		WaitForIPv6: waitForIPv6,
	}); err != nil {
		return nil, errors.Wrapf(
			err,
			"error getting cloud init metadata for proxmoxVM %s/%s",
			proxmoxVM.Namespace, proxmoxVM.Name)
	}
	return buf.Bytes(), nil
}

func GetOwnerProxmoxMachine(ctx context.Context, c client.Client, obj metav1.ObjectMeta) (*infrav1.ProxmoxMachine, error) {
	for _, ref := range obj.OwnerReferences {
		gv, err := schema.ParseGroupVersion(ref.APIVersion)
		if err != nil {
			return nil, err
		}
		if ref.Kind == "ProxmoxMachine" && gv.Group == infrav1.GroupVersion.Group {
			return getProxmoxMachineByName(ctx, c, obj.Namespace, ref.Name)
		}
	}
	return nil, nil
}

func getProxmoxMachineByName(ctx context.Context, c client.Client, namespace, name string) (*infrav1.ProxmoxMachine, error) {
	m := &infrav1.ProxmoxMachine{}
